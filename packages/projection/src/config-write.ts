...
// `readExecutorConfigLayers` in `@brambodev/session`.
//
// ponytail: `.brambo/config.json` is spelled here rather than imported from
// `@brambodev/session`, which spells it too and carries the same note. AD-2 forbids
 ...
 * reach '@brambodev/adapter-cli' (which owns EXECUTOR_CONFIG_KEY) under AD-2, and
 * importing '@brambodev/contracts' for METHOD_CONFIG_KEY alone would make one of the
 ...
 *
 * The write goes through `@brambodev/projection`'s `atomicWriteText` rather than a
