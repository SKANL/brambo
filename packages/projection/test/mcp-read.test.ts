...
//
// Every fixture below is written in the VENDOR's vocabulary, not brambo's, and
...
async function fileWith(name: string, body: string): Promise<string> {
  const root = await mkdtemp(join(tmpdir(), 'brambo-mcp-read-'))
  tempRoots.push(root)
...
describe('E1/E2: a vendor that has nothing to say contributes nothing, and is not an error', () => {
  ...
  it('reads an empty JSON document as empty rather than as malformed', async () => {
    ...
    // over a file brambo itself is happy to write into.
...
describe('E12: a server carrying more than brambo can hold is ingested for what brambo CAN hold', () => {
  it('names every dropped key, for every vendor, and never invents a root field for it', async () => {
    ...
    // brambo CAN hold. Everything above is an object, a number or a boolean, and
    ...
    // The entry is still CONTRIBUTED. Refusing it because brambo cannot carry an
    // env table would drop a working server over a key brambo never projects.
    for (const result of [claude, codex, opencode]) expect(result.unreadable).toEqual([])
  })
})

describe('E13 and its siblings: a native entry brambo cannot represent is reported, never guessed', () => {
  ...
  it('refuses a server whose `args` brambo cannot READ, rather than dropping them', async () => {
    // `args` is a field the registry DOES carry, so a value brambo cannot read is
    // not a key brambo cannot hold. Reporting it as dropped registered the server
  ...
  it("refuses a TOML value brambo did not write rather than guessing what it means", async () => {
...
describe.skipIf(process.platform === 'win32')('a config brambo may not OPEN is reported, not silently absent', () => {
...
describe('E7/E8: a document brambo cannot merge into is refused on the way IN too (D8)', () => {
  it('E7: names the line and column, through the same strategy the writer uses', async () => {
    ...
    expect(strict.code).toBe(BRAMBO_ERROR_CODES.projectionNativeMalformed)
    expect(strict.message).toMatch(/line \d+, column \d+/)
    expect(lenient.code).toBe(BRAMBO_ERROR_CODES.projectionNativeMalformed)
  ...
  it('E8: names the container key when it holds something brambo cannot address', async () => {
    ...
    expect(held.code).toBe(BRAMBO_ERROR_CODES.projectionNativeUnclaimable)
    expect(held.message).toContain('mcpServers')
    for (const error of [table, assigned]) {
      expect(error.code).toBe(BRAMBO_ERROR_CODES.projectionNativeUnclaimable)
      expect(error.message).toContain('mcp_servers')
    }
  })
})

describe('D4 case (ii): content brambo WOULD write is already satisfied, not a collision', () => {
  ...
  /** The document brambo itself produces for that entry, from an empty container. */
  async function brambosOwnOutput(target: ReturnType<typeof createClaudeMcpTarget>, seed: string, kind: RegistryEntriesByKind): Promise<string> {
  ...
  it.each([
  ...
  ] as const)('%s: an unclaimed entry identical to brambo’s own output is silent', async (_id, make, seed) => {
    const target = make()
    // The fixture is what brambo itself writes, so "the bytes would be the same
    // bytes" is MEASURED here rather than assumed — which is the claim the
    // original D4 asserted without executing it.
    const written = await brambosOwnOutput(target, seed, entries('npx', ['-y', 'x']))
    ...
    // NOT ADOPTED: brambo did not write those bytes, so it claims nothing. A
  ...
  it('is satisfied for an entry carrying a key brambo cannot represent', async () => {
