...
// `@brambo/environment`-only install cannot resolve `@brambo/registry`,
// `@brambo/projection` or `@brambo/kernel` under pnpm's strict layout, so a test
 ...
 * do, a third party can do by importing this package, with no `@brambo/cli`
 ...
 *
 * What makes it fail: move any part of the capability into `@brambo/cli` and this
...
describe('a consumer with no @brambo/cli installed', () => {
   ...
   *
   * What makes it fail: move the diagnosis into `@brambo/cli` and there is nothing
