...
// `@brambodev/environment`-only install cannot resolve `@brambodev/registry`,
// `@brambodev/projection` or `@brambodev/kernel` under pnpm's strict layout, so a test
 ...
 * do, a third party can do by importing this package, with no `@brambodev/cli`
 ...
 *
 * What makes it fail: move any part of the capability into `@brambodev/cli` and this
...
describe('a consumer with no @brambodev/cli installed', () => {
   ...
   *
   * What makes it fail: move the diagnosis into `@brambodev/cli` and there is nothing
