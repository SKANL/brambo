...
function workspaceImportsOf(files: readonly string[]): Set<string> {
  ...
  for (const file of files) {
    for (const specifier of importsOf(readFileSync(file, 'utf8'))) {
      if (specifier.startsWith('@brambodev/')) found.add(specifier)
    }
  }
  return found
}
...
/**
 * AD-2's topology is strictly downward. `@brambodev/environment` is CONSUMER tier,
 * exactly like `@brambodev/session`: it may depend on the kernel, the contracts and
 ...
 * comment, and pnpm would happily resolve an import of `@brambodev/cli` from the
 * very package whose reason to exist is that a third party does not need it.
 *
 * These clauses read IMPORT SPECIFIERS, so they see `@brambodev/x` and not a
 ...
 */
describe('@brambodev/environment dependency direction (AD-2)', () => {
  ...
  it('never reaches for @brambodev/cli or @brambodev/session, from src or from its own tests', () => {
    // The acceptance criterion is "a project that has NOT installed @brambodev/cli",
    // so a test importing the CLI would exercise the wrong claim; the scan
    // therefore covers `test` as well as `src`. `@brambodev/session` is barred for
    ...
    for (const file of files) {
      for (const specifier of importsOf(readFileSync(file, 'utf8'))) {
        expect(
          specifier.startsWith('@brambodev/cli') || specifier.startsWith('@brambodev/session'),
  ...
  it('keeps the diagnosis composed from init.ts, never from its own engine call', () => {
    ...
    for (const specifier of specifiers) {
      expect(
        specifier === '@brambodev/projection' || specifier === '@brambodev/registry',
  ...
  it('keeps the FR-29 consumer test importing NOTHING but this package', () => {
    ...
    // `consumer.test.ts` to take `RegistryStore` from `@brambodev/registry` and
    // `createMemoryLogSink` from `@brambodev/kernel` directly: every clause passed,
 ...
 * the ledger is the sole authority for what brambo may modify.
 *
 * `access` and `constants` are the two `brambo doctor` added, and both are pure
 * interrogation: `access(W_OK)` ASKS whether a write would be permitted, so a
 * diagnosis can stop promising a write brambo could not perform without trying
...
describe('@brambodev/environment writes no vendor file itself', () => {
  it('cannot reach the atomic writer that lands a vendor file', () => {
    // The other evasion a reviewer found: `import { atomicWriteText } from
    // '@brambodev/projection'` — a package this one already depends on, so no
