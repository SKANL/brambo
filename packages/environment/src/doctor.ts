...
import { BRAMBO_ERROR_CODES, REGISTRY_ENTRY_TYPES } from '@brambodev/contracts'
import type {
  DriftEntry,
  DriftKind,
  BramboErrorCode,
...
export type DiagnosisFindingKind =
   ...
   * rather than being discovered here. `@brambodev/environment` may not import a
 ...
 *
 * A STRUCTURAL shape, deliberately: `@brambodev/environment` may not import the
 * worktree implementation, and the one type both sides would otherwise share
 * would have to live in `@brambodev/contracts` — a third-party port surface, for a
