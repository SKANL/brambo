 ...
 * That is the whole reason it exists: `@brambodev/projection` needed the lockfile
 * protocol that lived in `@brambodev/registry`, and the `projection -> registry`
 ...
 *
 * They read IMPORT SPECIFIERS, so they see `@brambodev/x` and not a relative path
 * out of the package. That second route is closed repo-wide by the
 * `no-restricted-imports` regex in `eslint.config.js`.
 */
describe('@brambodev/lock is a leaf (AD-2)', () => {
  it('has sources to scan', () => {
    expect(sourceFiles.length).toBeGreaterThan(0)
    expect(testFiles.length).toBeGreaterThan(0)
  })

  it('declares exactly one dependency, and it is @brambodev/contracts', () => {
    expect([...declaredDependencies].sort()).toEqual(['@brambodev/contracts'])
  })
  ...
  it('reaches no @brambo package but @brambodev/contracts, from src OR from its own tests', () => {
    // `test` is scanned too. A leaf whose SOURCE is clean while its suite pulls
    // in `@brambodev/registry` still has the cycle — pnpm installs it, the module
    ...
    expect(reached.filter((specifier) => specifier !== '@brambodev/contracts')).toEqual([])
  })
})
