...
//
// This code was MOVED here from `@brambo/registry`, unchanged apart from its
// error codes and the word "registry" leaving its messages. It is a leaf: it
// depends on `@brambo/contracts` and nothing else (AD-2), so any package can
// serialize writes to a file without importing a sibling's domain — which is
// what `@brambo/projection`'s ledger needed and could not have.
//
...
// how `@brambo/registry` goes on raising exactly the two codes it always did.
//
// Staleness rules:
