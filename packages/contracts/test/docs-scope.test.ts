 ...
 * publish under.
 *
 * WHY THIS EXISTS. `M37.D` moved every package from `@brambodev/*` to
 * `@brambodev/*` because the `@brambodev` scope belongs to someone else. The
...
describe('the documentation names the scope the manifests publish under', () => {
  ...
  it('CONTROL: says nothing about a document that only names the declared scope', () => {
    ...
    expect(
      foreignScopeUses(
        [['README.md', 'Install `@brambodev/cli`, then `@brambodev/contracts` to implement a port.']],
        ['@brambodev'],
      ),
    ).toEqual([])
  })
