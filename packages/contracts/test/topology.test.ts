 ...
 * package-SPECIFIC clauses no generic checker can express — `@brambodev/environment`
 ...
 * GRAPH rather than about any package: strictly-downward `@brambodev/*` imports.
 ...
 * `_bmad-output/planning-artifacts/architecture/architecture-brambo-2026-08-24/ARCHITECTURE-SPINE.md`
 ...
 * - tier 1 — `@brambodev/lock`, a PRIMITIVE: the portable lockfile protocol, on
 *   `@brambodev/contracts` and nothing else. It earns a tier of its own rather than
 ...
 *   stopped being `@brambodev/registry`'s private machinery.
 ...
 * `@brambodev/contracts/validation` and `@brambodev/contracts` are one dependency.
 */
function packageNameOf(specifier: string): string | undefined {
  return specifier.startsWith('@brambodev/') ? specifier.slice('@brambodev/'.length).split('/')[0] : undefined
}
...
function violationsFor(packageName: string, dependencies: readonly string[]): string[] {
  ...
  if (tier === undefined) return [`@brambodev/${packageName} has no declared tier`]
  return dependencies.flatMap((dependency) => {
    const dependencyTier = TIER[dependency]
    if (dependencyTier === undefined) {
      return [`@brambodev/${packageName} (tier ${tier}) imports @brambodev/${dependency}, which the declared order does not name`]
    }
    ...
    return dependencyTier >= tier
      ? [
          `@brambodev/${packageName} (tier ${tier}) imports @brambodev/${dependency} (tier ${dependencyTier}) — imports must be strictly downward`,
        ]
      : []
  })
}
...
describe('package topology is strictly downward (AD-2)', () => {
  ...
  it('reports zero upward or sibling @brambodev/* imports across every package src', () => {
  ...
  it('flags an upward import, a sibling import, and an unknown package', () => {
    ...
    expect(violationsFor('contracts', ['session'])).toEqual([
      '@brambodev/contracts (tier 0) imports @brambodev/session (tier 3) — imports must be strictly downward',
    ])
    expect(violationsFor('contracts', ['kernel'])).toEqual([
      '@brambodev/contracts (tier 0) imports @brambodev/kernel (tier 0) — imports must be strictly downward',
    ])
    ...
    expect(violationsFor('lock', ['registry'])).toEqual([
      '@brambodev/lock (tier 1) imports @brambodev/registry (tier 2) — imports must be strictly downward',
    ])
  ...
  it('extracts a package name from every import form, subpaths included', () => {
    ...
    const kernel = `@brambodev/${'kernel'}`
    expect(
      [
        `import { a } from '${kernel}'`,
        `const m = await import("${kernel}")`,
        `export * from '${kernel}/deep/path'`,
        `import type { B } from '@brambodev/${'contracts'}/validation'`,
