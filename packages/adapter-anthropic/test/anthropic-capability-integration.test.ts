...
describe('Anthropic capability integration', () => {
  ...
  it('maps allowlisted hosted web and remote MCP only after selected egress grants', async () => {
    ...
    expect(result.status).toBe('ok')
    expect(requests[0]?.body).toMatchObject({ tools: [{ type: 'web_search_20250305' }, { type: 'mcp_toolset', mcp_server_name: 'docs', default_config: { enabled: false }, configs: { search: { enabled: true } } }], mcp_servers: [{ type: 'url', name: 'docs', url: 'https://example.com/mcp', authorization_token: 'mcp-private' }] })
