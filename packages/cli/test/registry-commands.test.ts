...
import { REGISTRY_ENTRY_TYPES, REMOVABLE_ENTRY_TYPES, RETIRED_ENTRY_TYPES } from '@brambodev/environment'
...
// exit codes. Which entries are VALID is `@brambodev/contracts` and is proven in
// `packages/contracts/test/registry.test.ts`; what a store does with them is
// `@brambodev/registry`'s. The rows below that end in a refusal therefore assert
...
// secret line falls is `@brambodev/registry`'s, proven in its own suite.
