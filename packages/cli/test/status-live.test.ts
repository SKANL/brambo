...
import type { UsageReport } from '@brambodev/session'
...
// fixtured.
//
// One live invocation, through the binary the way a user runs it: `brambo run`
// with no injected adapter and no injected spawner, then `brambo status`. Nothing
...
// BRAMBO_LIVE_STATUS=1 enables the check. The deterministic repository gate
...
// it with both variables repointed. Brambo's OWN scope is unaffected, because
// every `runBrambo` call below is handed an explicit `homeDir`; the only thing
...
async function tempDir(): Promise<string> {
  const dir = await mkdtemp(join(tmpdir(), 'brambo-status-live-'))
  roots.push(dir)
  return dir
}
...
async function probe(): Promise<{ available: boolean; reason: string }> {
  if (process.env['BRAMBO_LIVE_STATUS'] !== '1') {
    return { available: false, reason: 'BRAMBO_LIVE_STATUS=1 is required to enable the live status check' }
  }
...
describe('live: brambo status reports a real reading a real run produced', () => {
  it(
    ...
    async (ctx) => {
      ...
      expect(await runBrambo(['status'], { ...before, cwd, homeDir })).toBe(0)
      expect(before.err.join('\n')).toContain('claude-code: BRAMBO_USAGE_NOT_OBSERVED')

      // THE live invocation. No `createAdapter`, no `adapterOptions`: the
      // production path, exactly what a user's shell runs.
      const runIo = capture()
      const runCode = await runBrambo(['run', PROMPT], { ...runIo, cwd, homeDir })
      if (runCode !== 0) {
        // Never a failure of this suite: a refusal, an outage or an expired
        // credential is the provider's state, not brambo's defect.
        ctx.skip(
      ...
      expect(claude?.kind, `brambo status reported ${JSON.stringify(claude)}`).toBe('observed')
      ...
      // change to what brambo reports and must fail here rather than quietly
      ...
      for (const executorId of ['codex', 'opencode']) {
        expect(io.err.join('\n')).toContain(`${executorId}: BRAMBO_USAGE_NO_SURFACE`)
      }
    },
