...
import type { ResultEnvelope } from '@brambodev/contracts'
// From `@brambodev/session`, not `@brambodev/adapter-cli`: the CLI does not depend on
...
import type { ChildProcessSpawner, SpawnedChild, SpawnOutcome } from '@brambodev/session'
...
// configuration is usable belongs to `@brambodev/session` and is proven in
