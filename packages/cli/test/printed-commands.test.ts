...
import { DIAGNOSIS_FINDING_KINDS, FINDING_EXITS } from '@brambodev/environment'
...
//
// Every backtick-quoted `brambo …` string in the SHIPPED source of every package
...
// green:
//
//   (a) `brambo purge 'demo'` — a single-quoted argument, this repo's dominant
//       style, hid a fabricated verb inside `brambo add`'s own success output.
//   (b) `brambo remove --all` — the probe dispatched argv[0] alone, so a flag the
//       binary rejects passed.
//   (c) `packages/cli/bin/brambo.ts` ships, prints, and was never scanned.
//
...
/**
 * Strings that begin with the word `brambo` inside backticks and are NOT a
 * command the binary can be asked to run. Every one is listed deliberately.
 */
const NOT_A_COMMAND = new Map<string, string>([
  ['brambo has no adapter named \'<value>\'; available executors: <value>', 'the catalogue\'s error for an unknown executor id'],
  ['brambo <verb>', 'the GRAMMAR, written with a placeholder where a verb goes'],
  ["brambo will not write '<value>' because <value>", 'the config writer refusing to overwrite a document it cannot read'],
  ["brambo does not persist a '<value>' setting; it writes <value>", 'the config writer refusing a key outside its allowlist'],
  ["brambo could not load the method '<value>': <value>", 'the method resolver refusing a module specifier it could not import or validate'],
  ['brambo project <verb> [directory]', 'the project GRAMMAR, same placeholder'],
  ...
  ['brambo <value>', 'the machine branch of `verbAt`, with a placeholder where the verb goes'],
  ['brambo project <value>', 'the project branch of the same helper'],
  ['brambo run ... | head', 'a SHELL PIPELINE in a comment about EPIPE, not an argv'],
  ['brambo has no registry document at \'<value>\'', 'doctor\'s not-initialised detail'],
  ['brambo would rewrite \'<value>\' and the location is not writable', 'doctor\'s not-writable detail'],
  ['brambo reported no <value> finding in this run, so there is nothing for \'<value>\' to resolve; brambo never remediates a state it did not just report', 'a remediation refusal'],
  ['brambo knows no location where a previous build could have written into \'<value>\'', 'a discard refusal'],
  ['brambo could not tie <value> back to one projection target and one registry entry, so it will not act on it', 'a remediation refusal'],
  ['brambo wrote \'<value>\' to \'<value>\' and it is gone; brambo will not re-add it', 'the config target\'s removed-by-user detail'],
  ['brambo could not determine whether it is free (<value>)', 'the occupancy check\'s unclassifiable answer'],
  ['brambo could not determine whether these exist, so this is not evidence that nothing is installed: <value>', 'the undetermined-evidence sentence doctor prints; it WRAPPED across two source lines and was therefore invisible here until the unclosed-line guard forced it onto one'],
  ['brambo would not materialise \'<value>\' under \'<value>\' and holds no record of ever having done so, so there is nothing there for brambo to claim', 'an adopt refusal on a skills root'],
  ['brambo wrote \'<value>\' under \'<value>\' and it is gone; brambo will not re-add it', 'the skills target\'s removed-by-user detail'],
  ['brambo gains authority to overwrite AND to REMOVE exactly these path(s) on a later run: <value>', 'what adopt says before it claims'],
  ['brambo takes ownership of the <value> byte(s) now at \'<value>\' in \'<value>\'', 'what adopt describes'],
  ['brambo re-takes ownership of \'<value>\' in \'<value>\' at its CURRENT <value> byte(s), replacing the hash it held', 'what adopt describes on a re-claim'],
  ['brambo holds no claim for \'<value>\' at \'<value>\', so there is nothing to release', 'a release refusal'],
  ['brambo stops claiming \'<value>\' in \'<value>\'; whatever is there stays exactly as it is, and brambo will treat it as foreign until it is adopted again. Nothing on disk is removed by this or by any later run while the claim is gone', 'what release describes'],
  ['brambo cannot read any of \'<value>\' and will REPLACE it with an empty ledger: brambo then claims nothing at all, every entry it has written anywhere reports as a foreign collision, and each one has to be adopted back deliberately', 'what repair describes on a wholly unreadable ledger'],
  ['brambo rewrites \'<value>\' holding <value> record(s): <value> it can still vouch for<value>. Any record with no readable identity left is dropped, and whatever it claimed becomes yours to remove by hand', 'what repair describes'],
  ['brambo removes <value> from \'<value>\'; every other byte of the file is left exactly as it is', 'what discard describes'],
  ['brambo could not resolve an \'<value>\' selection through its configuration layers', 'the session\'s selection failure'],
  ['brambo has no workspace provider named \'<value>\'; available providers: <value>', 'the workspace catalogue\'s error for an unknown provider id -- the twin of the executor entry at the top of this list'],
])

/**
 * Commands brambo prints as COUNTER-EXAMPLES — spellings the binary refuses on
...
const REFUSED_ON_PURPOSE = new Map<string, string>([
  ['brambo project init -f', 'the COUNTER-EXAMPLE in run.ts: a single dash used to fall through as a positional and create a directory named -f'],
  ['brambo remove mcp-server --fs', 'the COUNTER-EXAMPLE in registry-commands.ts: the spelling that cannot name a dash-id, which is why -- exists'],
])
...
// `brambo` followed by a space or the closing tick, so `brambo-missing-x` is not
// one; and never across a newline, because a command lives on one line.
const PRINTED = new RegExp(TICK + 'brambo(?=' + TICK + '| )[^' + TICK + BACKSLASH + 'n]*' + TICK, 'g')
...
const UNCLOSED = new RegExp(TICK + 'brambo(?=' + TICK + '| )[^' + TICK + ']*$')
...
const probeDir = mkdtempSync(join(tmpdir(), 'brambo-printed-'))

/** Never spawns anything: `brambo run` is answered by this, not by an executor. */
...
describe('nothing brambo prints is a command brambo does not have', () => {
  it('scans every package, `bin/` included, and finds the commands to check', () => {
    ...
    expect(texts).toContain('brambo init')
    expect(texts).toContain('brambo remove <type> <id>')
    // From `bin/brambo.ts`, which the previous shape never opened.
    expect(texts).toContain('brambo run ... | head')
  })
  ...
  it('refuses a printed command wrapped across two source lines, which the scanner cannot see', () => {
    ...
    expect(
      wrapped,
      'a printed `brambo ...` command must live on ONE line or the invariant never sees it',
    ).toEqual([])
  })

  // THE SECOND SLOT, and it was blind. `FINDING_EXITS[kind].command` is what
  // `brambo doctor` prints as "To leave this state: ...", and it is an ordinary
  // single-quoted string -- so the backtick scanner above never saw it. A
  // planted `command: 'brambo evict-retired --all'` left this file 8/8 green
  ...
  it('dispatches every command `brambo doctor` names as an exit, whatever its quoting', async () => {
    let checked = 0
    for (const kind of DIAGNOSIS_FINDING_KINDS) {
      ...
      expect(tokens[0], kind).toBe('brambo')
      const path = verbPathOf(tokens)
      expect((await dispatch([...path, '--help'])).code, `${kind} names 'brambo ${path.join(' ')}'`).toBe(0)
  ...
  it('dispatches the VERB of every command it prints', async () => {
    ...
    for (const [text, command] of commands()) {
      ...
      if (code !== 0) failures.push(`'${text}' in ${command.file} names 'brambo ${path.join(' ')}'`)
    }
  ...
  it('(a) catches a fabricated verb hidden behind a quoted argument', async () => {
    const tokens = printedTokens("brambo purge 'demo'")
    expect(tokens).toEqual(['brambo', 'purge', "'demo'"])
