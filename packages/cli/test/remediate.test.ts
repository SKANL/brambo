...
// `brambo remediate` at the binary's edge: argv, output and exit codes.
//
// Every fact printed here is the capability's — which finding was selected, what
// would change, whether brambo refused. The CLI classifies nothing and writes
...
async function tempCwd(): Promise<string> {
  return mkdtemp(join(tmpdir(), 'brambo-cli-remediate-'))
}

/** A machine with claude-code present and one brambo-written entry the user edited. */
async function editedEntry(): Promise<string> {
  ...
  await store.dispose()
  const io = capture()
  await runBrambo(['init'], { ...io, homeDir })
  await writeFile(claudeJson, (await readFile(claudeJson, 'utf8')).replace('"ctx-server"', '"mine"'), 'utf8')
  return homeDir
}

/**
 * THE COMMAND BRAMBO PRINTS IS RUN, not read.
 *
 * Every clause in `printed-commands.test.ts` asks whether a printed verb
 * DISPATCHES, and `brambo remediate` dispatches perfectly — it simply answers
 * about the machine. At project scope brambo was naming commands that exit 1 or
 * do nothing:
 *
 *   project doctor on an edited entry  -> "`brambo remediate adopt` or `brambo remediate release`"
 *                                         both exit 1, REMEDIATION_REFUSED
 *   project doctor uninitialised       -> "`brambo init`", which exits 0 and
 *                                         leaves the project exactly as it was
 *
 * A string assertion would pin the spelling and prove nothing about the state.
 * These take the command out of brambo's own output and EXECUTE it.
 */
const PRINTED_COMMAND = /`(brambo [^`]+)`/g
...
describe('a command brambo prints at project scope resolves the project', () => {
  /**
   * THE EXIT, not any command in the paragraph.
   *
   * The first draft of this clause collected every `brambo …` in the whole report
   * and ran them all — and it PASSED, because the RESOLUTION half already names
   * both spellings ("`brambo init` (or `brambo project init`) creates brambo's
  ...
  it('names an init that actually initialises the project it was asked about', async () => {
    ...
    await runBrambo(['project', 'doctor', projectDir], { ...reported, homeDir })
    ...
    for (const text of named) {
      const ran = capture()
      await runBrambo(text.split(' ').slice(1), { ...ran, homeDir, cwd: projectDir })
    }
    const after = capture()
    await runBrambo(['project', 'doctor', projectDir], { ...after, homeDir })
    expect(
      after.out.join('\n'),
  ...
  it('tells a project claim which init will act on it', async () => {
    ...
    const io = capture()
    await runBrambo(['project', 'add', 'mcp-server', 'ctx', '--command', 'ctx-server', projectDir], { ...io, homeDir })
    await runBrambo(['project', 'init', projectDir], { ...io, homeDir })
    ...
    await runBrambo(['project', 'remediate', 'adopt', projectDir, '--entry', 'ctx'], { ...described, homeDir })
    ...
    // The sentence used to name `brambo init` alone. Driven before the fix: the
    // hand-edited byte survived that command and died to `brambo project init`.
    expect(said, 'the consequence named only the machine init').toContain('brambo project init')
  })
  ...
  it('names a remediation the project scope will accept', async () => {
    ...
    for (const text of named) {
      const ran = capture()
      const code = await runBrambo(text.split(' ').slice(1), { ...ran, homeDir, cwd: projectDir })
      expect(
        [code, ran.err.join(' ')],
        `brambo printed '${text}' and running it was refused`,
      ).not.toContain('BRAMBO_PROJECTION_REMEDIATION_REFUSED')
      expect(code, `brambo printed '${text}' and running it exited ${String(code)}`).not.toBe(1)
    }
  })
})

describe('brambo remediate', () => {
  it('describes without writing by default, and performs only with --apply', async () => {
    ...
    const described = capture()
    expect(await runBrambo(['remediate', 'adopt', '--executor', 'claude-code', '--entry', 'ctx'], {
      ...described,
      homeDir,
    })).toBe(0)
    ...
    const ledgerBefore = await readFile(join(homeDir, '.brambo', 'projection-ledger.json'), 'utf8')

    const applied = capture()
    expect(await runBrambo(['remediate', 'adopt', '--executor=claude-code', '--entry=ctx', '--apply'], {
      ...applied,
      homeDir,
    })).toBe(0)
    expect((JSON.parse(applied.out.join('\n')) as { outcome: { applied: boolean } }).outcome.applied).toBe(true)
    expect(applied.err.join('\n')).toContain('changed:')
    expect(await readFile(join(homeDir, '.brambo', 'projection-ledger.json'), 'utf8')).not.toBe(ledgerBefore)
  ...
  it('exits 1 on a refusal and prints the findings the user could have named', async () => {
    ...
    expect(await runBrambo(['remediate', 'adopt', '--apply'], { ...io, homeDir })).toBe(1)
    expect(io.err.join('\n')).toContain('BRAMBO_PROJECTION_REMEDIATION_REFUSED')
  ...
  it('prints the WHOLE synopsis on a usage error, not a prefix of it', async () => {
    ...
    // pushed `brambo --help` off the end of it — silently, for six pre-existing
    // usage-error paths, which then advertised `brambo remediate` without
    // `brambo project remediate` and no longer said help existed.
    const io = capture()
    expect(await runBrambo(['bogus'], { ...io, homeDir: await tempCwd() })).toBe(2)
    ...
    for (const line of [
      ...
      'brambo project init',
      'brambo doctor',
      'brambo project doctor',
      'brambo remediate',
      'brambo project remediate',
      'brambo --help',
    ]) {
  ...
  it('answers --help wherever it appears, and takes a directory for the project scope', async () => {
    const io = capture()
    // Not only as the FIRST option token: `brambo run` accepts it anywhere and
    // two answers to one question is how a binding drifts.
    expect(await runBrambo(['remediate', 'adopt', '--apply', '--help'], { ...io, homeDir: await tempCwd() })).toBe(0)
    expect(io.out.join('\n')).toContain('brambo project remediate')
    ...
    expect(
      await runBrambo(['project', 'remediate', 'release', projectDir, '--apply'], { ...scoped, homeDir }),
    ).toBe(1)
    expect(JSON.parse(scoped.out.join('\n'))).toMatchObject({ scope: 'project' })
  })

  it('answers --help and advertises every remediation brambo has', async () => {
    for (const argv of [
      ['remediate', '--help'],
      ['project', 'remediate', '-h'],
    ]) {
  ...
  it('runs against the project scope when asked, without touching the machine one', async () => {
    ...
    expect(await runBrambo(['project', 'remediate', 'release', '--apply'], { ...io, homeDir, cwd: projectDir })).toBe(1)
  ...
  it('removes a legacy block only when asked, by name', async () => {
    ...
    expect(await runBrambo(['doctor'], { ...doctor, homeDir })).toBe(1)
    expect(doctor.err.join('\n')).toContain('legacy-block')

    const io = capture()
    expect(await runBrambo(['remediate', 'discard', '--executor', 'claude-code', '--apply'], { ...io, homeDir })).toBe(0)
    expect(JSON.parse(await readFile(settings, 'utf8'))).toEqual({ model: 'sonnet' })

    const after = capture()
    expect(await runBrambo(['doctor'], { ...after, homeDir })).toBe(1)
