...
import { scopeDirectory, setConfigValue } from '@brambodev/environment'
import { verbAt } from './registry-commands.ts'
import { resolveExecutor, resolveMethod } from '@brambodev/session'
...
//
// Thin, like every other binding here: the write is `@brambodev/environment`'s, the
// id check and the effective selection are `@brambodev/session`'s. This file parses
...
export async function runSwap(
  ...
  // `@brambodev/environment`'s own index calls it "the trust boundary that
