  ...
  if (argv[0] === 'project') {
    ...
    if (argv[1] === 'doctor') {
      return await runDoctor(argv.slice(2), out, err, 1, async (directory) => {
        ...
        // handed in because `@brambodev/environment` may not import a workspace
 ...
 * workspace capabilities in `@brambodev/session`, print what they did, map it to an
 ...
 * argv, call the capability in `@brambodev/environment`, print its diagnosis, map
 ...
 * call the capability in `@brambodev/environment`, print its result, map it to an
