...
//
// DUPLICATED, deliberately: `@brambodev/contracts` enforces the same rule on a MethodPlugin's `version`
// and carries its own copy, because AD-1 forbids this package a runtime dependency on anything —
// `@brambodev/contracts` included. `packages/contracts/test/method.test.ts` asserts the two agree on
