...
describe('@brambo/kernel zero-dependency invariant', () => {
  ...
  it('never imports @brambo/contracts from kernel sources (AD-1)', () => {
    ...
    for (const file of sources) {
      for (const specifier of importsOf(readFileSync(file, 'utf8'))) {
        expect(specifier.startsWith('@brambo/contracts'), `${file} imports '${specifier}'`).toBe(false)
      }
    }
  })
  ...
  it('never imports outside the kernel package through relative paths (AD-1)', () => {
    ...
    for (const file of sources) {
      for (const specifier of relativeImportsOf(readFileSync(file, 'utf8'))) {
        ...
        expect(escapesPackage, `${file} imports '${specifier}' which resolves outside @brambo/kernel`).toBe(false)
      }
    }
  })
