...
// single public entry. A `@brambodev/session`-only install cannot resolve
// `@brambodev/contracts` or `@brambodev/kernel` under pnpm's strict layout, so a test
 ...
 * one `brambo run` prints. If composition drifts back into `@brambodev/cli`, this test
