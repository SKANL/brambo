...
import type { ExecutorAdapter, ResultEnvelope, WorkspaceHandle, WorkspaceProvider } from '@brambodev/contracts'
import {
  createKernel,
  createMemoryLogSink,
  lostRecordCount,
...
import { createExecutorPlugin } from '@brambodev/adapter-cli'
import { createWorkspacePlugin } from '@brambodev/workspace-local'
import {
  createSessionKernel,
...
describe('createSessionKernel is the only composition surface', () => {
  it('exports no factory that yields a kernel, a plugin or an adapter', async () => {
    // The measured hole this closes: `@brambodev/session` re-exported `createKernel`
    ...
    // green, because the thin-binding pin scanned for `@brambodev/kernel` by name.
    //
    ...
    // MethodPlugin (a manifest, which `@brambodev/contracts` already validates in
    ...
    // import the kernel: `packages/cli` depends on `@brambodev/environment` and
    // `@brambodev/session` and nothing else.
    //
