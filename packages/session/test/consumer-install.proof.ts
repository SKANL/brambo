...
import { WORKSPACE_CLAUSES } from '@brambodevdev/contracts'
 ...
 * every `@brambodevdev/*` name below is a directory to pack or a tarball to read.
 */
 ...
 * long as every package was `@brambodevdev/<dir>`. The scope is now `@brambodev` when
...
/**
 * Every workspace package `@brambodevdev/cli` needs at RUNTIME, walked from the
 ...
 * `@brambodevdev/projection` shipped a probe that asked the weaker question and let CI
 ...
 * `import '@brambodevdev/registry'` throws on its FIRST line while every
 ...
 * `@brambodevdev/session` by Node's own rules, out of `node_modules`.
 *
...
const CONSUMER_SCRIPT = `import { createMemoryLogSink, resolveExecutor, runSession } from '@brambodevdev/session'
...
console.log(
  JSON.stringify({
    ...
    resolvedFrom: import.meta.resolve('@brambodevdev/session'),
  }),
)
 ...
 */
const CONSUMER_TYPES = `import { runSession } from '@brambodevdev/session'
import type { ResultEnvelope, SessionOptions } from '@brambodevdev/session'
 ...
 * only `@brambodevdev/contracts`." The session arm above installs nine tarballs, so it
 ...
 * port is implementable installing ONLY `@brambodevdev/contracts`; FR-9 says a
...
console.log(
  JSON.stringify({
    ...
    resolvedFrom: import.meta.resolve('@brambodevdev/contracts'),
...
const CONSUMER_TSCONFIG = JSON.stringify(
  {
    compilerOptions: {
      ...
      // `@brambodevdev/contracts/dist/executor.d.ts` needs an ambient `AbortSignal`
...
describe.skipIf(OPT_OUT)('a project OUTSIDE the workspace that installed the packed tarballs', () => {
  beforeAll(async () => {
    ...
    // the entire point: inside the workspace, pnpm answers `@brambodevdev/session` from
  ...
  it('packs EVERY workspace package, because the list above says it does', async () => {
    ...
    // `@brambodevdev/session` and its dependency closure, so a package can be packed,
  ...
  it('runs a real session and returns the envelope brambo run prints', () => {
    // Byte-for-byte the object `@brambodevdev/cli` hands to `JSON.stringify(_, null, 2)`,
    // and the input to its exit-code ternary — asserted from a project that has
    // no `@brambodevdev/cli` installed and no access to this workspace.
  ...
  it('exports dist from the INSTALLED manifest, with the workspace protocol resolved away', () => {
    ...
    expect(installedManifest['dependencies']).toEqual({
      '@brambodevdev/adapter-cli': WORKSPACE_VERSION,
      '@brambodevdev/contracts': WORKSPACE_VERSION,
      '@brambodevdev/kernel': WORKSPACE_VERSION,
      '@brambodevdev/sandbox': WORKSPACE_VERSION,
      '@brambodevdev/workspace-git-worktree': WORKSPACE_VERSION,
      '@brambodevdev/workspace-local': WORKSPACE_VERSION,
    })
  })
  ...
  it('installs the @brambodevdev/cli TARBALL alone and runs the binary a user would get', async () => {
    ...
    // be unreachable from the binary", and `@brambodevdev/cli` was exactly that -- the
    ...
    // makes a `@brambodevdev/*` name unable to resolve from a registry -- but the
      ...
      ...new Set(
        closure.flatMap((dir) =>
          Object.entries(
            ...
            .filter(([name]) => !name.startsWith('@brambodevdev/'))
  ...
  it('installs and imports @brambodevdev/contracts ALONE, and a port compiles against it', async () => {
      ...
      `${JSON.stringify(
        {
          ...
          private: true,
          type: 'module',
          dependencies: { '@brambodevdev/contracts': `file:../project/tarballs/${tarballName('contracts')}` },
        },
        null,
        2,
      )}\n`,
    ...
    expect(readme, '@brambodevdev/contracts shipped no README.md in its tarball').toBeDefined()
    ...
    // dependency appearing on `@brambodevdev/contracts` fails HERE, loudly, instead of
    ...
    // asked: one `@brambodevdev/*` package arrived, not a closure.
  ...
  it('packs a binary that is emitted JavaScript with its shebang intact', () => {
    ...
    // workspace-side assertion noticing. No import of `@brambodevdev/cli` is involved,
  ...
  it('ships every manifest entry point AND every module those entry points reach', () => {
    ...
    // `@brambodevdev/registry` and got a green 7/7 beside an import that threw. So the
    ...
    for (const packageDir of PACKAGE_DIRS) {
      ...
      expect(targets.length, `@brambodevdev/${packageDir} names no shippable target`).toBeGreaterThan(0)
      for (const target of targets) {
        for (const absent of unreachable(entries, packedPath(target))) {
          missing.push(`@brambodevdev/${packageDir} does not ship ${absent}, reached from ${target}`)
        }
      }
    }
  ...
  it('declares @brambodevdev/* dependency ranges the packed versions actually satisfy', () => {
    // The consumer installs every `@brambodevdev/*` as a direct `file:` dependency,
    ...
    // LENIENT one: a manifest requiring `"@brambodevdev/kernel": "^9.9.9"` beside a
    ...
    for (const [packageDir, entries] of packed) {
      ...
      expect(manifest.name, `packages/${packageDir} packed under an unexpected name`).toBe(`@brambodevdev/${packageDir}`)
    ...
    for (const [packageDir, entries] of packed) {
      ...
      for (const [name, range] of Object.entries(dependencies)) {
        if (!name.startsWith('@brambodevdev/')) continue
        const packedVersion = version.get(name)
        if (range !== packedVersion) {
          wrong.push(`@brambodevdev/${packageDir} requires ${name}@${range}, but it packs as ${String(packedVersion)}`)
        }
      }
    }
 ...
 * re-exports nothing is its own failure mode, and `@brambodevdev/cli` legitimately
...
describe('what a consumer gets when they bundle the published packages', () => {
  ...
  it('bundles and RUNS every package, and the table is what it was', async () => {
    ...
    for (const packageDir of PACKAGE_DIRS) {
      ...
      expect(defaultExport, `@brambodevdev/${packageDir} has no default export target`).toBeDefined()
