...
/**
 * AD-2's topology is strictly downward. `@brambodev/session` is CONSUMER tier: it may
 ...
 * of `@brambodev/cli` from the very package whose reason to exist is that a third
 * party does not need the CLI.
 *
 * These clauses read IMPORT SPECIFIERS, so they see `@brambodev/x` and not a relative
 ...
 */
describe('@brambodev/session dependency direction (AD-2)', () => {
  ...
  it('never reaches for @brambodev/cli, from src or from its own tests', () => {
    ...
    for (const file of files) {
      for (const specifier of importsOf(readFileSync(file, 'utf8'))) {
        expect(specifier.startsWith('@brambodev/cli'), `${file} imports '${specifier}'`).toBe(false)
      }
    }
  })
