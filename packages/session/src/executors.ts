...
import {
  ...
  unknownExecutor,
} from '@brambodev/adapter-cli'
import type { CliExecutorAdapterOptions, ShippedExecutor } from '@brambodev/adapter-cli'
import { METHOD_CONFIG_KEY, BRAMBO_ERROR_CODES, BramboError, isRecord } from '@brambodev/contracts'
import { createLayeredConfig, deepMerge } from '@brambodev/kernel'
import type { ConfigLayer, LayeredConfig } from '@brambodev/kernel'

// Executor SELECTION: which shipped adapter this run uses, decided through the
// layered configuration brambo already owns.
//
// The catalogue itself moved to `@brambodev/adapter-cli` with Story M3.B — the
...
// `@brambodev/session` is the FR-29 surface: a consumer that installed only this
...
// `@brambodev/environment`, which owns the same `<scope>/.brambo` convention. That
// package is CONSUMER tier and so is this one, and `packages/session/test/
// guard.test.ts` pins @brambodev/session's dependency set to exactly four packages —
// so reaching for it would be an AD-2 violation the gate rejects, not a reuse.
// Upgrade path: move the scope-directory convention down into `@brambodev/contracts`
...
export interface ExecutorSelection {
   ...
   * to render one; `@brambodev/cli` does not print it, because on the one path where
 ...
 * refused with the same code `@brambodev/environment` refuses it with.
 */
...
export interface ExecutorConfigLayers {
   ...
   * still override them. `@brambodev/session` puts its computed workspace root here
 ...
 *
 * Ships from `@brambodev/session` beside `runSession`, so FR-29 holds: a third party
