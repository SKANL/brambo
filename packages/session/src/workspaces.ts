...
import { createGitWorktreeWorkspacePlugin } from '@brambodev/workspace-git-worktree'
import { WORKSPACE_CONFIG_KEY, createWorkspacePlugin } from '@brambodev/workspace-local'
import type { WorkspacePlugin } from '@brambodev/workspace-local'
...
/** The `git worktree`-backed provider (`@brambodev/workspace-git-worktree`). */
...
function unusableSelection(detail: string): BramboError {
  ...
  // (see its note in `@brambodev/contracts`). There is no workspace twin of
