...
import type { DelegationRecord, ResultEnvelope, SessionEventLog } from '@brambodev/contracts'
...
export function createSessionReceipt(options: SessionReceiptOptions): ReviewReceipt {
  ...
  return createReceipt(options.target, options.result, options.issuedAt, events, options.delegations)
}
