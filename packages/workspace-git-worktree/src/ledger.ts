 ...
 * interleave — the same reasoning `@brambodev/registry`'s lockfile rests on, whose
 ...
 *
 * Taken from `@brambodev/registry`'s `DEFAULT_MAX_AGE_MS` rather than chosen here,
...
export class WorktreeLedger {
  ...
  async readIntent(id: string): Promise<WorktreeRemovalIntent | undefined> {
    ...
    // so the age fallback gives it the same grace `@brambodev/registry` gives a
   ...
   * `@brambodev/registry`'s lock accepts, with the identical consequence (a state
   ...
   *
   * NOT `@brambodev/projection`'s `atomicWriteText`, and deliberately so: that one
 ...
 * user — alive. Only ESRCH proves it is gone, which is `@brambodev/registry`'s rule
