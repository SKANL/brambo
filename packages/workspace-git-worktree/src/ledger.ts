 ...
 * interleave — the same reasoning `@brambo/registry`'s lockfile rests on, whose
 ...
 *
 * Taken from `@brambo/registry`'s `DEFAULT_MAX_AGE_MS` rather than chosen here,
...
export class WorktreeLedger {
  ...
  async readIntent(id: string): Promise<WorktreeRemovalIntent | undefined> {
    ...
    // so the age fallback gives it the same grace `@brambo/registry` gives a
   ...
   * `@brambo/registry`'s lock accepts, with the identical consequence (a state
   ...
   *
   * NOT `@brambo/projection`'s `atomicWriteText`, and deliberately so: that one
 ...
 * user — alive. Only ESRCH proves it is gone, which is `@brambo/registry`'s rule
