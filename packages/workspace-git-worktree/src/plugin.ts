...
import type { StandardSchemaResult, WorkspaceProvider } from '@brambodev/contracts'
import { isNonEmptyString, isRecord, issue } from '@brambodev/contracts/validation'
import type { ActivationContext, PluginFactory, PluginManifest } from '@brambodev/kernel'
...
//
// It mirrors `@brambodev/workspace-local`'s plugin deliberately: same service, same
...
// and `selectWorkspaceProvider` in `@brambodev/session` decides which.
//
// The three constants below are RESTATED rather than imported from
// `@brambodev/workspace-local`. They are kernel-service vocabulary, not that
 ...
 * `configSchema` (`@brambodev/kernel`'s `lifecycle.ts`), so a plugin that registers
 ...
 * plugin alone — measured against `@brambodev/workspace-local` as its control, which
 ...
 *
 * Colliding with `@brambodev/workspace-local`'s id is not a hazard: both provide the
...
// complaining about its own vocabulary (`@brambodev/workspace-local` carries the
 ...
 *
 * Same shape and same leniency as `@brambodev/workspace-local`'s, and for the same
...
export interface GitWorktreeWorkspacePluginOptions {
   ...
   * a directory brambo CREATES and a user may legitimately relocate; the repo is
   ...
   * `@brambodev/workspace-local` would then warn about a key brambo itself defined.
   */
 ...
 * same "directory brambo puts workspaces under" the key already means, answered
 ...
 * kernel's `PluginFactory` is SYNCHRONOUS (`@brambodev/kernel`'s `lifecycle.ts`) and
...
export function createGitWorktreeWorkspacePlugin(
  ...
  const factory: PluginFactory = (context: ActivationContext) => {
    ...
    // kernel's work — and unlike `@brambodev/registry`, which merges factory options
