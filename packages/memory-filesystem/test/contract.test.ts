...
import { MEMORY_CLAUSES, MEMORY_FORMAT_VERSION, runMemoryContractSuite } from '@brambo/contracts'
import type { MemoryContractHarness, SuiteReport } from '@brambo/contracts'
...
async function harness(): Promise<MemoryContractHarness> {
  ...
  return {
    providerName: '@brambo/memory-filesystem',
...
describe('FilesystemMemoryProvider against the memory contract suite', () => {
  ...
  it('names the provider in every violation, so a red clause says which one broke', async () => {
    ...
    for (const violation of report.violations) {
      expect(violation.detail).toContain('@brambo/memory-filesystem')
    }
  })
})
