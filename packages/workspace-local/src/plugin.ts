...
// test suite in `@skanl/brambo-contracts` is written against the port itself. What
...
// `provider` is not read here — `selectWorkspaceProvider` in `@skanl/brambo-session`
 ...
 * reason `@skanl/brambo-registry` states for its own second call. The kernel checks the
...
export interface WorkspacePluginOptions {
   ...
   * the document supplies one. `@skanl/brambo-session` puts its own computed root into
 ...
 * in `@skanl/brambo-session` for the same reason it is stated on `createProvider`: the
...
export function createWorkspacePlugin(options: WorkspacePluginOptions = {}): WorkspacePlugin {
  ...
  const factory: PluginFactory = (context: ActivationContext) => {
    ...
    // plugin has; `@skanl/brambo-session` forwards these to its `onWarning` seam and
