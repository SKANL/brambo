...
export default tseslint.config(
  ...
  },
  {
    ...
    // planted in @skanl/brambo-cli through `../../workspace-local/src/index.ts` with the
  ...
  {
    // `@skanl/brambo-cli` is argv, formatting and exit codes. It has no business
    ...
    // list and `@skanl/brambo-*` import specifiers, and owning selection needs neither,
    ...
    rules: {
      'no-restricted-imports': [
        ...
        {
          paths: [
            { name: 'node:fs', message: '@skanl/brambo-cli reads no files; the capability packages do (thin-binding pin)' },
            { name: 'node:fs/promises', message: '@skanl/brambo-cli reads no files; the capability packages do (thin-binding pin)' },
            { name: 'fs', message: '@skanl/brambo-cli reads no files; the capability packages do (thin-binding pin)' },
            { name: 'fs/promises', message: '@skanl/brambo-cli reads no files; the capability packages do (thin-binding pin)' },
          ],
          patterns: [
            {
              regex: String.raw`^\.\.[\\/]\.\.`,
            ...
            },
            {
              ...
              // above), and Story M3.B by RE-EXPORT — `@skanl/brambo-session` briefly
              // re-exported `createKernel` and both plugin factories, and a
              // complete working session composition was planted in
              // `packages/cli/src/` importing only `@skanl/brambo-session`, with
              ...
              //
              // So the restriction is on the NAMES, from any `@skanl/brambo-*` module.
              ...
              message:
                '@skanl/brambo-cli composes nothing: it may not hold a kernel, mount a plugin, resolve a service or build an adapter, whichever package re-exports the capability (thin-binding pin, AD-2)',
            },
          ],
        },
      ],
    ...
    rules: {
      'no-restricted-imports': [
        'error',
        {
          patterns: [
            {
              group: ['@skanl/brambo-contracts', '@skanl/brambo-contracts/*'],
              message: 'the kernel never imports @skanl/brambo-contracts (AD-1)',
            },
            {
              regex: String.raw`^\.\.[\\/]\.\.`,
              message:
                'relative imports must stay inside @skanl/brambo-kernel; cross-package imports are forbidden (AD-1)',
            },
          ],
        },
      ],
