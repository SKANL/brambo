...
`@skanl/panda-sandbox` es la capa neutral del SDK para seleccionar un sandbox, crear una sesión administrada y validar el límite alrededor de la ejecución. **No** implementa aislamiento del sistema operativo por sí misma.
...
```ts
import { createSandboxProviderResolver } from '@skanl/panda-sandbox'
...
Usa `@skanl/panda-sandbox-local` para proveedores conservadores respaldados por el host o `@skanl/panda-sandbox-remote` para un transporte remoto inyectado.
