...
`@skanl/panda-sandbox` is the provider-neutral SDK layer for selecting a sandbox, creating a managed session, and validating the boundary around execution. It does **not** implement operating-system isolation itself.
...
Use `@skanl/panda-sandbox-local` for conservative host-backed providers or `@skanl/panda-sandbox-remote` for an injected remote transport.
