...
Installing `@brambodev/cli` is not required — that is the point of this package. Nor
is installing `@brambodev/contracts` or `@brambodev/kernel`: every type and helper the
...
- **Interrupts.** There is no default — a library that installs
  ...
  wire your own (`@brambodev/cli` passes its SIGINT/SIGTERM registration).
...
```ts
import { createMemoryLogSink, runSession, SESSION_ACTION_ID } from '@brambodev/session'
...
```ts
import { createMemoryLogSink, createSessionKernel, runSession } from '@brambodev/session'
