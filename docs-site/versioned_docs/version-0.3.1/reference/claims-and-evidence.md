...
audience: Developers, maintainers, and reviewers evaluating brambo
...
This page is the evidence matrix for brambo's public claims. It separates an executable repository guarantee from a documented design intention, a measured but bounded result, and an administrative or external fact that this repository cannot prove by itself.
...
| **Typed errors:** callers can route on stable `BramboError` codes rather than parse messages. | VERIFIED | `packages/contracts/src/errors.ts` defines `BramboError` and `BRAMBO_ERROR_CODES`; `packages/contracts/test/errors.test.ts` checks the stable shape and naming convention; provider tests such as `packages/workspace-local/test/provider.test.ts` assert real coded failures. | A code is stable only when it is part of the exported contract and its behavior remains covered; free-form error text is not a routing API. |
| **Projection ownership:** brambo records ownership in its ledger and does not claim an invented marker namespace inside vendor files. | VERIFIED | `packages/contracts/test/projection.test.ts` rejects the retired marker vocabulary, versions the ledger, and checks target/file/native-location/content-hash authority; `packages/projection/test/ledger.test.ts`, `packages/projection/test/drift.test.ts`, and `packages/projection/test/native-projection.test.ts` exercise ledger, drift, and native-location behavior. | The guarantee is about brambo's ownership decisions and writes. It does not mean brambo controls files or locations that the vendor does not read. |
| **Sandbox and platform limits:** policy validation is explicit, path containment is checked, and host implementations are tested per platform. | PARTIAL / MEASURED | `packages/contracts/test/sandbox.test.ts` drives policy, capability, path, snapshot, resource-limit, and exact-argv validation; `packages/sandbox-local/test/host-conformance/linux.test.ts`, `macos.test.ts`, and `windows.test.ts` cover host-specific behavior; `.github/workflows/sandbox-conformance.yml` runs the conformance suites on their native substrates. | Brambo does **not** claim universal OS-level isolation, unrestricted vendor-process control, or identical enforcement on every host. A host or vendor capability must be treated as unavailable when it cannot be proved. |
| **Integrated live executor evidence:** real executor verification for this plan uses only opt-in OpenCode with the explicitly configured DeepSeek V4.1 Flash model. | PARTIAL / MEASURED | `packages/adapter-cli/test/opencode-live.test.ts` requires `BRAMBO_LIVE_OPENCODE=1` and `BRAMBO_OPENCODE_MODEL=opencode-go/deepseek-v4.1-flash`, probes only `opencode`, and skips when unavailable. | No live test claim is made when the binary, credentials, or exact model configuration is unavailable; deterministic fakes cover default checks. |
...
- **UNVERIFIED:** Vendor executor behavior, authentication, rate limits, or permission defaults outside brambo's adapter and contract tests.
