...
skill and mcp-server entry envelopes (defined in `@skanl/brambo-contracts`), with machine-scoped write serialization via a hand-rolled
...
Mounts as a real plugin on the `@skanl/brambo-kernel` lifecycle via
